...
namespace YIUIFramework
{
    public partial class YIUILoopScrollMulti<TData>
    {
        ...
        /// 列表元素被点击的事件
        /// </summary>
        public delegate void OnClickItemEvent(int index, TData data, UIBase item, bool select);
        ...
        public YIUILoopScrollMulti<TData> SetOnClickInfo(
        ...
        //传入对象 选中目标
        public void OnClickItem(UIBase item)
        {
        ...
        private void OnClickItem(int index, UIBase item, bool select)
        {
        ...
        private UIBase AddOnClickEvent(UIBase uiBase)
        {
            ...
            {
                Debug.LogError($"当前监听的事件未找到 请检查 {nameof(UIBase)} 中是否有这个事件 {m_ItemClickEventName}");
            }
            else
            {
