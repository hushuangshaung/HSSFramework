...
namespace UniFramework.Tween
{
	...
	public static class UniTween
	{
		...
		{
			UniTimer timer = UniTimer.CreateRepeatTimer(delay, interval);
			TimerNode node = new TimerNode(timer);
			node.SetTrigger(trigger);
			return node;
		}
		...
		{
			UniTimer timer = UniTimer.CreateRepeatTimer(delay, interval, duration);
			TimerNode node = new TimerNode(timer);
			node.SetTrigger(trigger);
			return node;
		}
		...
		{
			UniTimer timer = UniTimer.CreateRepeatTimer(delay, interval, maxRepeatCount);
			TimerNode node = new TimerNode(timer);
			node.SetTrigger(trigger);
			return node;
		}
